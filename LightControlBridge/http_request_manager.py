...
class HttpRequestManager:
	...
	def _set_params(self):
		try:
			...
			self._send_response(400, b'Bad Request 2')
			return

		for k, v in params.items():
			if not isinstance(v, int) or not k in "ArRgGbBsSdpv":
				self._send_response(400, b'Bad Request 3')
				return
	...
	def _report_parse_error(self):
		self._send_response(400, b'Bad Request 1')
	...
	def _parse_header(self, buf, idx, length):
		...
		if eol_idx == idx or buf[eol_idx - 1] != b'\r':
