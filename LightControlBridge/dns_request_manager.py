...
class DnsRequestManager:
	...
	def _handle_new_connection(self):
		req, addr = self._sock.recvfrom(1458)
		if not 16 < len(req):
			return  # too short
		...
		resp = array.array('B', req[:q_end])

		qr = resp[2] & 0x80
		opcode = (resp[2] >> 3) & 0x15
		qcount = resp[4] * 256 + resp[5]
		qtype = resp[name_end + 1] * 256 + resp[name_end + 2]
		qclass = resp[name_end + 3] * 256 + resp[name_end + 4]
		...
		self._sock.sendto(bytearray(resp) + rest, addr)
